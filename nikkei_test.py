...
def score_nikkei_article(title, is_china_up_close=False):
    ...
    important_keywords = [
        ...
        "semiconductors", "pmi", "cpi", "housing", "foreign禁止:foreign direct investment", "tech",
    ...
    score = 1 if is_china_up_close else 0  # Mindestscore für China Up Close
    ...
    if any(kw in title_lower for kw in footer_phrases):
        score = 0
    print(f"DEBUG - score_nikkei_article: Titel '{title[:50]}...': Score {score}, China: {has_china}, Japan: {has_japan}, China Up Close: {is_china_up_close}")
    return max(score, 0)
        ...
        for eid in email_ids[:5]:
            ...
            for link_tag in links:
                ...
                if "nikkei.com" not in final_url.lower():
                    ...
                    continue
                score = score_nikkei_article(final_title, is_china_up_close=True)
                if score > 0:
                    scored_posts.append((score, f'• <a href="{final_url}">{final_title}</a>'))
...
def main():
    ...
    # China Up Close (nur donnerstags)
    if datetime.now().weekday() == 3:  # Donnerstag
