...
def fetch_economic_calendar():
    ...
    try:
        ...
        # Header
        header = (
            f"{'Tag'.ljust(col_widths['weekday'])} | "
            f"{'Datum'.ljust(col_widths['date'])} | "
            f"{'Event'.ljust(col_widths['event'])} | "
            f"{'Organisation'.ljust(col_widths['org'])} | "
            f"{'Priorität'.ljust(col_widths['prio'])}"
        )
        separator = (
            f"{'-'*col_widths['weekday']} | "
            f"{'-'*col_widths['date']} | "
            f"{'-'*col_widths['event']} | "
            f"{'-'*col_widths['org']} | "
            f"{'-'*col_widths['prio']}"
        ...
        markdown = ["### 📅 Was wichtig wird:", "", header, separator]
        ...
        for date_obj, group in grouped:
            ...
            for _, row in group.iterrows():
                event = str(row['Event'])[:col_widths['event']].ljust(col_widths['event'])
                org = str(row['Organisation'])[:col_widths['org']].ljust(col_widths['org'])
                prio = str(row['Priority'])[:col_widths['prio']].ljust(col_widths['prio'])
                line = (
                    f"{weekday.ljust(col_widths['weekday'])} | "
