...
import pandas as pd# CEST ist UTC+2
cest = timezone(timedelta(hours=2))# Logging einrichten, nur auf Konsole, keine Dateien
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(levelname)s - %(message)s',
    handlers=[logging.StreamHandler()]
)
logger = logging.getLogger()warnings.filterwarnings("ignore", category=XMLParsedAsHTMLWarning)# Pfad zu den Holiday JSON Dateien, CPR-Cache, Economic Calendar CSV und Cache-Dateien
...
IACI_CACHE_FILE = os.path.join(FREIGHT_CACHE_DIR, "iaci_cache.json")# Erstelle den freight_indicies-Ordner, falls er nicht existiert
os.makedirs(FREIGHT_CACHE_DIR, exist_ok=True)def load_holidays(filepath):
    ...
    try:
        ...
        return set()  # Fallback: Leere Liste, um fortzufahrendef is_holiday(today_str, holidays_set):
    return today_str in holidays_setdef is_weekend():
    return date.today().weekday() >= 5# CPR-Cache laden
def load_cpr_cache():
...
def fetch_scfi():
    ...
    return scfi_value, None, scfi_date, warning_message# Warn-E-Mail senden (für SCFI, WCI und IACI)
def send_warning_email(warning_message, index_name="SCFI"):
    ...
    try:
        ...
        if not config:
            ...
            raise Exception("Missing CONFIG")    pairs = config.split(";")
...
def load_wci_cache():
    ...
    try:
        ...
        cache = {}
        save_wci_cache(cache)
        return cache# WCI-Cache speichern
...
def load_iaci_cache():
    ...
    try:
        ...
        cache = {}
        save_iaci_cache(cache)
        return cache# IACI-Cache speichern
    ...
    try:
        ...
        if os.path.exists(IACI_CACHE_FILE):
            try:
                ...
                logger.error("Corrupted IACI cache file, overwriting with new data")    existing_cache.update(cache)
...
def fetch_wci_email():
    ...
    try:
        ...
        if not env_vars:
            ...
            raise Exception("DREWRY not set")    gmail_user = None
    gmail_pass = None
    ...
    if 'mail' in locals():
        mail.logout()
    return None, None# IACI-E-Mail abrufen
def fetch_iaci_email():
    ...
    if 'mail' in locals():
        mail.logout()
    return None, None# WCI aus HTML extrahieren
def extract_wci_from_html(html_content, subject):
    """Extrahiert den WCI-Wert und das Datum aus dem HTML-Inhalt."""
    try:
        soup = BeautifulSoup(html_content, 'html.parser')
        wci_text = soup.get_text(strip=True)    wci_match = re.search(r'\$(\d{1,3}(,\d{3})*)\s*per 40ft container', wci_text)
    if not wci_match:
    ...
    return None, None# IACI aus HTML extrahieren
def extract_iaci_from_html(html_content, subject):
    """Extrahiert den IACI-Wert und das Datum aus dem HTML-Inhalt."""
    try:
        soup = BeautifulSoup(html_content, 'html.parser')
        iaci_text = soup.get_text(strip=True)    iaci_match = re.search(r'\$(\d{1,3}(,\d{3})*)\s*per 40ft container', iaci_text)
    if not iaci_match:
...
def fetch_wci():
    ...
    return wci_value, None, wci_date, warning_message# IACI-Daten abrufen
def fetch_iaci():
    ...
    return iaci_value, None, iaci_date, warning_message# Werte vorladen (global)
today_str = date.today().isoformat()
...
is_weekend_day = is_weekend()# === Wirtschaftskalendar ===
def fetch_economic_calendar():
    ...
    try:
        if not os.path.exists(ECONOMIC_CALENDAR_FILE):
            ...
            return ["###  Was wichtig wird:", "", " No calendar data available (file not found)."]    df = pd.read_csv(ECONOMIC_CALENDAR_FILE, encoding="utf-8")
    ...
    # Rendering-Option: "bold" für ** (Buttondown), "arrow" für  (Testumgebung), "html" für <b>
    ...
    for date_obj, group in grouped:
        ...
        if highlight_style == "bold":
            ...
            date_line = f" {date_text}" if date_obj.date() == today else date_text
    ...
    return ["###  Was wichtig wird:", "", " Error fetching calendar data."]# ===  Konfiguration aus ENV-Variable ===
config = os.getenv("CONFIG")
if not config:
    raise ValueError("CONFIG environment variable not found!")
pairs = config.split(";")
config_dict = dict(pair.split("=", 1) for pair in pairs)# === Google Mapping ===
source_categories = {
...
def score_article(title, summary=""):
    ...
    for word in negative_keywords:
        if word in content:
            score -= 3
    return score# === News-Artikel filtern & bewerten ===
def fetch_news(feed_url, max_items=20, top_n=5):
    ...
    return [item[1] for item in scored[:top_n]] or ["Keine aktuellen China-Artikel gefunden."]# === SCMP & Yicai Ranking-Wrapper ===
def fetch_ranked_articles(feed_url, max_items=20, top_n=5):
    return fetch_news(feed_url, max_items=max_items, top_n=top_n)# === Extract_source (für Google News) ===
def extract_source(title):
    for source in source_categories:
        if f"– {source}" in title or f"- {source}" in title or title.lower().endswith(source.lower()):
            return source
    return "Unknown Source"# === Substack aus E-Mails abrufen ===
def fetch_substack_from_email(email_user, email_password, folder="INBOX", max_results_per_sender=5):
    ...
    try:
        ...
        if duplicates:
            print(f" Warning: Duplicate email addresses in substacks.json: {duplicates}")
        ...
        substack_senders = []
        posts.append(("Allgemein", " Fehler: substacks.json ungültig.", "#", "", 999))for attempt in range(3):
    try:
...
return posts if posts else [("Allgemein", "Keine neuen Substack-Mails gefunden.", "#", "", 999)]# === Caixin Newsletter ===
...
def score_caixin_article(title):
    title_lower = title.lower()
    score = 0# Schlüsselwörter für China-Bezug
must_have_in_title = [
    "china", "chinese", "xi", "beijing", "shanghai", "hong kong", "taiwan", "prc",
    "communist party", "cpc", "byd", "alibaba", "tencent", "huawei", "li qiang", "brics",
...
return max(score, 0)def fetch_caixin_from_email(email_user, email_password, folder="INBOX", max_results=5):
    ...
    imap.logout()
    return []# === China Update aus YT abrufen ===
...
def fetch_latest_nbs_data():
    ...
    try:
        ...
        return [f" Fehler beim Abrufen der NBS-Daten: {e}"]# === Börsendaten & Wechselkurse abrufen ===
def fetch_index_data():
    ...
    return results# Interpretation für USD/CNY-Spread
def interpret_usd_cny_spread(spread_pips):
    if spread_pips <= -100:
        ...
        return "CPR stark über Markterwartungen: Markt drängt auf Yuan-Stärke"# Interpretation für CNH–CNY-Spread
def interpret_cnh_cny_spread(spread_pips):
    if spread_pips <= -50:
        ...
        return "Starke CNY-Abwertung"def fetch_cpr_forexlive():
    ...
    return None, None, Nonedef fetch_cpr_from_x():
    ...
    return None, None, Nonedef fetch_cpr_usdcny():
    print("DEBUG - fetch_cpr_usdcny: Starting to fetch CPR")
    cpr_cache = load_cpr_cache()
    today_str = date.today().isoformat()
    yesterday_str = (date.today() - timedelta(days=1)).isoformat()
    print(f"DEBUG - fetch_cpr_usdcny: Today: {today_str}, Yesterday: {yesterday_str}")print("DEBUG - fetch_cpr_usdcny: Trying CFETS")
...
print(" DEBUG - fetch_cpr_usdcny: CFETS failed, trying ForexLive")
...
print(" DEBUG - fetch_cpr_usdcny: ForexLive failed, trying X posts")
...
print(" DEBUG - fetch_cpr_usdcny: All sources failed, using cache or Reuters estimate")
...
return None, 7.1820, None, prev_cprdef fetch_currency_data():
    ...
    headers = {"User-Agent": "Mozilla/5.0"}
    results = {}cpr, estimate, pips_diff, prev_cpr = fetch_cpr_usdcny()
if cpr is not None:
...
x_accounts = [
    ...
    return [f"• {name} (@{account}) → {url}"]# === Render Markdown für Substack ===
def render_markdown(posts):
    ...
    return markdown# === Briefing generieren ===
...
def send_briefing():
    ...
    briefing_content = generate_briefing()msg = MIMEText(briefing_content, "html", "utf-8")
...
try:
    ...
    print(f" ERROR - send_briefing: Failed to send email: {str(e)}")# === Hauptskript ===
